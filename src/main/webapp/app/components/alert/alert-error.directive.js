(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('rethinkdbApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-rethinkdbApp-error');
                var entityKey = httpResponse.headers('X-rethinkdbApp-params');
                if (errorHeader) {
